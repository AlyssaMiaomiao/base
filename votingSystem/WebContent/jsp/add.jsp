...
<div id="voteManage" class="box" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc3-0" data-genuitec-path="/VoteSite/WebRoot/add.jsp">
	<h2>添加新投票</h2>
	<div class="content">
	<form method="post" action="${pageContext.request.contextPath}/addSubject">
			<dl>
				<dt>投票内容：</dt>
				<dd>
				...
				<dt>投票类型：</dt>
				<dd>
		  		   <input type="radio" name="voteType"  <c:if test="${subject.number==1 }"> checked="checked" </c:if> value="1" />单选
				   <input type="radio" name="voteType"  <c:if test="${subject.number==2 }"> checked="checked" </c:if> value="2" />多选
				</dd>
				...
				<dd id="voteoptions">
					<c:forEach items="${subject.options}" var="option" varStatus="st">
						<p><input type="text" class="input-text" name="options" value="${option.content }"/>
						<c:if test="${st.index>1}"><a class="del" href="javascript:;" onclick="delOption()">删除</a></c:if></p>
					</c:forEach>
				</dd>
				</c:if>
				<dt></dt>
